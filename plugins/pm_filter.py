...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("🚫Link Expired, Request Again ♻", show_alert=True)
        return
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="❓How To Download❓", url='https://telegram.me/FHDmovies24x7Official/3415')
        ]
    )
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('Checking in Database ♻️')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('💎 VIP', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
