...
async def gen_link_batch(bot, message):
    if " " not in message.text:
        return await message.reply("Use Correct Format.\nExample ›› <code>/batch https://t.me/FHDmovies24x7official/1000 https://t.me/FHDmovies24x7official/1001</code>.")
    ...
    try:
        ...
        return await message.reply('This Is Private Channel, Make Me Admin In The Channel To Index The Files')
    except (UsernameInvalid, UsernameNotModified):
        return await message.reply('Invalid Link specified.')
    except Exception as e:
        return await message.reply(f'Errors - {e}')

    sts = await message.reply("<b>Generating Link 🌀</b>")
    if chat_id in FILE_STORE_CHANNEL:
        string = f"{f_msg_id}_{l_msg_id}_{chat_id}_{cmd.lower().strip()}"
        b_64 = base64.urlsafe_b64encode(string.encode("ascii")).decode().strip("=")
        return await sts.edit(f"<b>🎰 Here Is Your Link👇</b> \nhttps://telegram.me/{temp.U_NAME}?start=DSTORE-{b_64}")

    FRMT = "<b>Generating Link 🌀\nTotal Messages › `{total}`\nDone › `{current}`\nRemaining › `{rem}`\nStatus › `{sts}`</b>"
